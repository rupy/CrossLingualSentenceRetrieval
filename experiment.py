...
class Experiment:
    ...
    def fit_changing_step(self,mode='PART', line_flag=False, start_step=1, end_step=25, every_nth=1):

        for s in range(start_step, end_step, every_nth):
            self.joint.gcca_fit(mode=mode, line_flag=line_flag, step=s)
            self.joint.cca_fit(mode=mode, line_flag=line_flag, step=s)
    ...
    def calc_accuracy_changing_step(self, mode='PART', line_flag=False, reg_param=0.1, start_step=1, end_step=25, every_nth=1):
        ...
        for step in step_list:
            self.joint.gcca_transform(mode=mode, line_flag=line_flag, step=step, reg_param=reg_param)
            self.joint.cca_transform(mode=mode, line_flag=line_flag, step=step, reg_param=reg_param)
    ...
    def fit_chenging_regparam(self, reg_params, step=5):
        for r in reg_params:
            self.joint.gcca_fit(mode='PART', line_flag=False, step=step, reg_param=r)
            self.joint.cca_fit(mode='PART', line_flag=False, step=step, reg_param=r)

    def calc_accuracy_changing_reg_params(self, reg_list, mode='PART', line_flag=False, step=5):

        res_cca_data = []
        res_gcca_data = []

        for reg in reg_list:
            self.joint.gcca_transform(mode=mode, line_flag=line_flag, step=step, reg_param=reg)
            self.joint.cca_transform(mode=mode, line_flag=line_flag, step=step, reg_param=reg)
